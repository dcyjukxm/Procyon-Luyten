...
public final class ArrayUtilities
{
    ...
    static {
        GLOBAL_ARRAY_TYPE_CACHE = Cache.createTopLevelCache();
        ARRAY_TYPE_CACHE = Cache.createThreadLocalCache(ArrayUtilities.GLOBAL_ARRAY_TYPE_CACHE);
    }
    ...
    public static Object copyOf(final Object array, final int newLength, final Class<?> newType) {
        final Object copy = (newType == Object[].class) ? new Object[newLength] : Array.newInstance(newType.getComponentType(), newLength);
    ...
    public static <T> Class<T[]> makeArrayType(final Class<T> elementType) {
        final Class<?> arrayType = ArrayUtilities.ARRAY_TYPE_CACHE.get(elementType);
        if (arrayType != null) {
            return arrayType;
        }
        return (Class<?[]>)ArrayUtilities.ARRAY_TYPE_CACHE.cache(elementType, Array.newInstance(elementType, 0).getClass());
    }
    ...
    public static <T> T[] insert(final T[] array, final int index, final T... values) {
        ...
        final Object[] newArray = (Object[])Array.newInstance(array.getClass().getComponentType(), array.length + newItemCount);
        System.arraycopy(array, 0, newArray, 0, index);
        final int remaining = array.length - index;
        if (remaining > 0) {
    ...
    public static <T> T[] append(final T[] array, final T value) {
        ...
        final Object[] newArray = (Object[])Array.newInstance(value.getClass(), 1);
        newArray[0] = value;
        return (T[])newArray;
    }
    ...
    public static <T> T[] append(final T[] array, final T... values) {
        ...
        final Object[] newArray = (Object[])Array.newInstance(values.getClass().getComponentType(), values.length);
    ...
    public static <T> T[] removeAll(final T[] array, final T... values) {
        ...
        final Object[] newArray = (Object[])Array.newInstance(array.getClass().getComponentType(), array.length - matchCount);
        int sourcePosition = 0;
        for (int j = 0; j < matchCount; ++j) {
            final int matchIndex = matchIndices[j];
            if (matchIndex == Integer.MAX_VALUE) {
                break;
            }
            System.arraycopy(array, sourcePosition, newArray, sourcePosition - j, matchIndex);
    ...
    public static <T> T[] removeFirst(final T[] array, final T value) {
        final int index = indexOf((T[])VerifyArgument.notNull((T[])array, "array"), value);
        if (index == -1) {
            return array;
        }
    ...
    public static <T> T[] removeLast(final T[] array, final T value) {
        final int index = lastIndexOf((T[])VerifyArgument.notNull((T[])array, "array"), value);
        if (index == -1) {
            return array;
        }
        return remove(array, index);
    ...
    public static <T> T[] retainAll(final T[] array, final T... values) {
        ...
        Arrays.sort(matchIndices);
        final Object[] newArray = (Object[])Array.newInstance(array.getClass().getComponentType(), matchCount);
        int j = 0;
        int k = 0;
        while (j < count) {
            final int matchIndex = matchIndices[j];
            if (matchIndex != Integer.MAX_VALUE) {
                newArray[k++] = array[matchIndex];
            }
            ++j;
        }
        return (T[])newArray;
    ...
    public static <T> T[] union(final T[] array, final T... values) {
        ...
        if (matchCount == 0) {
            return (T[])append(array, (Object[])values);
        }
        final Object[] newArray = Arrays.copyOf(array, array.length + values.length - matchCount);
        int j = 0;
        int k = array.length;
        while (j < count) {
            final int matchIndex = matchIndices[j];
    ...
    public static <T> List<T> asUnmodifiableList(final T... items) {
        return new UnmodifiableArrayList<T>(items, null);
    }
    ...
    private static final class UnmodifiableArrayList<T> extends AbstractList<T>
    {
        ...
        private UnmodifiableArrayList(final T[] array) {
            super();
            this._array = VerifyArgument.notNull(array, "array");
        }
